...
2. 一覧内の物件詳細リンクをページネーション経由で全ページから抽出する
3. 詳細画面 HTML を R2 に保存する
4. 詳細画面の「3点セットのダウンロード」ボタンをクリックして PDF を取得し、R2 に保存する
5. 保存した PDF と一覧由来の所在地・座標 (`lat`/`lng`) を含む一覧 JSON (`items.json`) を詳細 HTML と同じ日付階層に保存する
6. `SCRAPE_MAX_DETAILS` で処理件数を制限する
...
```text
bit/okayama/html/by-sha256/<sha256>.html
bit/okayama/pdf/by-sha256/<sha256>.pdf
bit/okayama/html/YYYY/MM/DD/items.json
```

HTML と PDF は取得したバイト列の SHA-256 ハッシュをキーにして保存します。同じ内容の HTML/PDF がすでに R2 に存在する場合は再アップロードせず、日付別の `items.json` から既存キーを参照します。`items.json` の `items[].html` / `items[].pdf` には R2 上の保存キーを記録し、`items[].htmlDownloadedDate` / `items[].pdfDownloadedDate` にはスクレイピングで取得した日付を記録します。地図表示は `items[].lat` / `items[].lng` を優先し、座標がない過去 manifest や欠損データのみ `/api/geocode` を fallback として利用します。
...
| `SCRAPE_MAX_DETAILS` | 任意 | 最大取得件数。空ならMAX。定期実行では全件取得し、手動実行時のみ `max_details` 入力で制限します。 |
