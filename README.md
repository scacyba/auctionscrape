...
bit/okayama/pdf/YYYY/MM/DD/001.pdf
bit/okayama/html/YYYY/MM/DD/items.json
```

HTML の `detail-stable-id` は詳細URLのパスとURLハッシュから生成します。PDF は一覧の処理順に `001.pdf`, `002.pdf` ... として保存し、`items.json` の `items[].pdf` には R2 上の保存キーを記録します。地図表示は `items[].lat` / `items[].lng` を優先し、座標がない過去 manifest や欠損データのみ `/api/geocode` を fallback として利用します。
