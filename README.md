...
5. 保存した PDF の一覧 JSON (`items.json`) を詳細 HTML と同じ日付階層に保存する
...
HTML の `detail-stable-id` は詳細URLのパスとURLハッシュから生成します。PDF は一覧の処理順に `001.pdf`, `002.pdf` ... として保存し、`items.json` の `items[].pdf` には R2 上の保存キーを記録します。
